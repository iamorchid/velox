...
void AllocationPool::maybeGrowLastAllocation(uint64_t bytesRequested) {
  ...
  if (updateOffset > endOfReservedRun()) {
    ...
    // 这里会增加ContiguousAllocation的size_大小, 同时为新增的大小进行reserver操作
...
void AllocationPool::newRunImpl(MachinePageCount numPages) {
  if (usedBytes_ >= hugePageThreshold_ ||
    ...
    //
    // 对于ContiguousAllocation(即对应大块内存), 虽然请求的内存大小numPages, 但实际上
    // 会向os分配更大的虚拟内存, 此时os并没有为这些内存分配物理内存, 同时memory pool仅仅
    // 为numPages的内存进行过reserve操作, 调用方只允许访问numPages指定的内存大小. 
    //
    // 当调用方需下次要继续分配内存时, 此时可以不需要再次向os申请虚拟内存了, 而可以直接使用
    // 之前预分配好的虚拟内存, 但调用方在真正使用这些内存之前, 也必须进行reserver操作, 避
    // 免超过它的内存quota.
    // 
    auto range = largeAlloc.hugePageRange().value(); // 虚拟内存的实际range
    startOfRun_ = range.data();                      // 虚拟内存的start
    bytesInRun_ = range.size();                      // 虚拟内存的实际大小(比如numPages大)
    currentOffset_ = 0;
