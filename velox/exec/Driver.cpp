...
void BlockingState::setResume(std::shared_ptr<BlockingState> state) {
  ...
  std::move(state->future_)
      ...
      .thenValue([state](auto&& /* unused */) {
        ...
        // [question]
        // driver->state().isTerminated为true时, 也需要入队?
...
StopReason Driver::runInternal(
  ...
  // task()->enter(...) 会检查task是否已经stop, 如果没有则将当前driver设置为onThread
  StopReason stop =
  ...
  CancelGuard guard(self, task().get(), &state_, [&](StopReason reason) {
    ...
    // reason 为 StopReason::kNone, 表示本driver自己执行遇到异常; 否则, 意味着task
    // 已经被cancel (其他task失败或者本task的其他driver失败导致).
...
void Driver::run(std::shared_ptr<Driver> self) {
  ...
  //
  // [question]
  // 如果在if之后, task执行了terminate, 当前driver其实还是会进入wait. 这种情况下,
  // 会存在问题吗？如果去掉这里的if判断, 会有问题吗?
  //
